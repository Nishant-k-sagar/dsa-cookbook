...
async function regenerateContent(topicSlug: string): Promise<void> {
  console.log(`\n=== Regenerating content for: ${topicSlug} ===`);
  ...
  for (let i = 0; i < seedFile.problems.length; i++) {
    ...
    // NO EXISTENCE CHECK - Always regenerate
  ...
  if (generatedProblems.length === 0) {
    console.error('No problems generated. Aborting.');
    process.exit(1);
  }
  ...
  if (fs.existsSync(existingPath)) {
    try {
      ...
      if (existing.topic?.summary && existing.topic.summary.trim().length > 0) {
        console.log(`Topic "${topicSlug}" already has content, skipping topic content generation.`);
        topicContent = {
          ...existing.topic,
          id: topicSlug,
          title: seedFile.topic,
          problemCount: generatedProblems.length
        };
      } else {
        // Generate topic-level content using Mistral AI
        topicContent = await generateTopicContent(aiClient, seedFile.topic, topicSlug, generatedProblems);
      }
    } catch {
      // Generate topic-level content using Mistral AI
      topicContent = await generateTopicContent(aiClient, seedFile.topic, topicSlug, generatedProblems);
  ...
  console.log(`\nWritten regenerated content to: ${generatedPath}`);
}
...
async function main() {
  ...
  if (!topicArg) {
    console.log('Usage:');
    console.log('  npx tsx regenerateContent.ts --topic=<topic-slug>');
    console.log('');
    console.log('Examples:');
    console.log('  npx tsx regenerateContent.ts --topic=graph');
    console.log('  npx tsx regenerateContent.ts --topic=binary-search');
    console.log('');
    console.log('Note: This script ALWAYS regenerates content (no skipping).');
    console.log('      Use contentGenerator.ts for normal generation with skipping.');
    process.exit(1);
  }

  const topicSlug = topicArg.replace('--topic=', '').replace(/^["']|["']$/g, '');
  await regenerateContent(topicSlug);
